...
````{r}
library(dplyr)
rename(Western_Europe, "Homicide" = "Homicide rate in Europe over long-term (per 100,000) (homicides per 100,000 people)")



```

##Creation og new facet
...
Western_Europe %>%
  ggplot( aes(x=Year, y=`Homicide rate in Europe over long-term (per 100,000) (homicides per 100,000 people)`, group=Entity)) +
    geom_area() +
    theme(legend.position="none") +
    ggtitle("Evolution of homicide rates across western europe") +
    theme(
...
# See the solution made above


##Moving the legend of the facet

4) Move the legend from the current position on the side to below the facets, and label it "Country" instead of "Entity".
```{r}
#I've chosen not to have a legend, but to place the entity of each graph on top if it. 
#Allthough, a way to move to the legend to the bottom would be to do as following: theme(legend.position = "bottom")
```

##Creation of floating table of content in Rmarkdown
